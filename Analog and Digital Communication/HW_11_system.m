...
j=1;                #counter
n=50;               #Error Energy samples for average ¿¿¿How many samples???
...
err_energy=5*epsilon_er; #I initialize the energy error by 5 times the minimum
e(1:n)=5*epsilon_er;     #Initialize the vector for store the error 

targ_n=[zeros(9,1);1;0.5;1]; #Defining my target channel. 
f_split=256;        #number of divisions in frequency
####
#Option
only_real=true; #if I put true is only real binary sequence as input, false then imaginary binary sequence
...
###############
##Filter adaptation
  ye=wn'*I(1:L); #estimated output 
  testye(j)=ye; #just for testing
  y=xn'*I(1:C);  #real output
  testy(j)=y;
  e(j+n)=y-ye;   #error in this iteration
  wn=wn+e(j+n)*I./(norm(I)^2); #adapting the filter
  err_energy=(1/n)*dot(e(length(e)-n:length(e)),e(length(e)-n:length(e)));#calculating the error energy in n samples
