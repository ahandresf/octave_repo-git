...
###For the model #Theta[n]=(0.99^i)*Theta[n-1]+U[n] where U[n] is mean zero and var_u=0.01
clear i;
for i=1:10
  A(i)=0.99^i
endfor
A=A' #A is a vector of 10x1. (0.99^i)
B=ones(np,1);
...
while(epsilon_er>0.00001 && iteration!=max_num_iterations)
  ...
  x=voice_frame(1+(iteration*160):((iteration*160)+160));#move my voice 160 frames with every iteration
